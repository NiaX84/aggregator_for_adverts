...
class ReadJsonFileTest(TestCase):
    ...
    def test_read_json_files_from_web_missing_values_csv_output(self):
        response = self.client.post(reverse('zillion_aggregator:read_json_no_grouping'), {'files': "https://api.apify.com/v2/datasets/KR4HaLLWPX7F6Qxse/items?format=json&clean=1&attachment=1"})
        self.assertEqual(response.status_code, 200)
        result = json.loads(response.content, encoding='utf-8')
